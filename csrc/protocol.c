...
int write_preflight_ok(FILE* writer, const PreflightOk* msg) {
    ...
    DEBUG_LOG(2, "DEBUG: Protocol: Writing PreflightOK message (space=%llu)\n", 
    ...
    DEBUG_LOG(2, "DEBUG: Protocol: Writing type=%d, len=%u, space_be=0x%016llx\n", 
            type, len, (unsigned long long)space_be);
    
    if (fwrite(&type, 1, 1, writer) != 1) {
        DEBUG_LOG(2, "DEBUG: Protocol: Failed to write type (errno: %d)\n", errno);
        return -1;
    }
    if (fwrite(&len_be, 4, 1, writer) != 1) {
        DEBUG_LOG(2, "DEBUG: Protocol: Failed to write length (errno: %d)\n", errno);
        return -1;
    }
    if (fwrite(&space_be, 8, 1, writer) != 1) {
        DEBUG_LOG(2, "DEBUG: Protocol: Failed to write space (errno: %d)\n", errno);
        return -1;
    }
    
    DEBUG_LOG(2, "DEBUG: Protocol: PreflightOK message written, flushing\n");
    if (fflush(writer) != 0) {
        DEBUG_LOG(2, "DEBUG: Protocol: Failed to flush PreflightOK (errno: %d)\n", errno);
        return -1;
    }
    DEBUG_LOG(2, "DEBUG: Protocol: PreflightOK flushed successfully\n");
    return 0;
}
...
int write_transfer_result(FILE* writer, const TransferResult* msg) {
    ...
    DEBUG_LOG(2, "DEBUG: Protocol: Writing TransferResult message (ok=%d, bytes=%llu)\n", 
    ...
    DEBUG_LOG(2, "DEBUG: Protocol: Writing type=%d, len=%u, ok=%d, bytes_be=0x%016llx\n", 
    ...
    if (fwrite(&ok, 1, 1, writer) != 1) {
        DEBUG_LOG(2, "DEBUG: Protocol: Failed to write ok flag (errno: %d)\n", errno);
        return -1;
    }
    if (fwrite(&bytes_be, 8, 1, writer) != 1) {
        DEBUG_LOG(2, "DEBUG: Protocol: Failed to write bytes (errno: %d)\n", errno);
        return -1;
    }
    
    DEBUG_LOG(2, "DEBUG: Protocol: TransferResult written, flushing\n");
    if (fflush(writer) != 0) {
        DEBUG_LOG(2, "DEBUG: Protocol: Failed to flush TransferResult (errno: %d)\n", errno);
        return -1;
    }
    DEBUG_LOG(2, "DEBUG: Protocol: TransferResult flushed successfully\n");
    return 0;
}
...
int read_transfer_result_full(FILE* reader, TransferResult* msg) {
    ...
    DEBUG_LOG(2, "DEBUG: Protocol: Reading TransferResult message (full)\n");
    
    uint8_t msg_type;
    if (read_message_type(reader, &msg_type) != 0) {
        DEBUG_LOG(2, "DEBUG: Protocol: Failed to read message type (errno: %d)\n", errno);
        return -1;
    }
    DEBUG_LOG(2, "DEBUG: Protocol: Read message type: %d\n", msg_type);
    
    if (msg_type != MSG_TRANSFER_RESULT) {
        DEBUG_LOG(2, "DEBUG: Protocol: Expected MSG_TRANSFER_RESULT (%d), got %d\n", 
    ...
    if (read_message_length(reader, &msg_len) != 0) {
        DEBUG_LOG(2, "DEBUG: Protocol: Failed to read message length (errno: %d)\n", errno);
        return -1;
    }
    DEBUG_LOG(2, "DEBUG: Protocol: Read message length: %u\n", msg_len);
...
int read_transfer_result_payload(FILE* reader, TransferResult* msg) {
    ...
    DEBUG_LOG(2, "DEBUG: Protocol: Reading TransferResult payload\n");
    uint8_t ok;
    uint64_t bytes_be;
    
    // Read payload only - header should already be read by caller
    if (fread(&ok, 1, 1, reader) != 1) {
        DEBUG_LOG(2, "DEBUG: Protocol: Failed to read ok flag (errno: %d, ferror: %d, feof: %d)\n", 
                errno, ferror(reader), feof(reader));
        return -1;
    }
    DEBUG_LOG(2, "DEBUG: Protocol: Read ok flag: %d\n", ok);
    
    if (fread(&bytes_be, 8, 1, reader) != 1) {
        DEBUG_LOG(2, "DEBUG: Protocol: Failed to read bytes (errno: %d, ferror: %d, feof: %d)\n", 
                errno, ferror(reader), feof(reader));
        return -1;
    }
    DEBUG_LOG(2, "DEBUG: Protocol: Read bytes value (raw: 0x%016llx)\n", 
            (unsigned long long)bytes_be);
    
    msg->ok = ok != 0;
    msg->received_bytes = my_ntohll(bytes_be);
    DEBUG_LOG(2, "DEBUG: Protocol: TransferResult payload read successfully (ok=%d, bytes=%llu)\n",
            msg->ok, (unsigned long long)msg->received_bytes);
    return 0;
}
...
int read_message_type(FILE* reader, uint8_t* type) {
    ...
    DEBUG_LOG(2, "DEBUG: Protocol: Reading message type\n");
    size_t bytes_read = fread(type, 1, 1, reader);
    if (bytes_read != 1) {
        if (feof(reader)) {
            DEBUG_LOG(2, "DEBUG: Protocol: End of stream reached (connection closed)\n");
        } else {
            DEBUG_LOG(2, "DEBUG: Protocol: Failed to read message type (bytes_read=%zu, errno=%d, ferror=%d, feof=%d)\n", 
...
int read_message_length(FILE* reader, uint32_t* length) {
    ...
    DEBUG_LOG(2, "DEBUG: Protocol: Reading message length\n");
    uint32_t len_be;
    if (fread(&len_be, 4, 1, reader) != 1) {
        DEBUG_LOG(2, "DEBUG: Protocol: Failed to read message length (errno=%d, ferror=%d, feof=%d)\n", 
                errno, ferror(reader), feof(reader));
        return -1;
    }
    
    *length = ntohl(len_be);
    DEBUG_LOG(2, "DEBUG: Protocol: Read message length: %u (raw: 0x%08x)\n", *length, len_be);
    return 0;
}
